...
test.describe("Invalid City Names - Malformed City Names with Special Characters", () => {
  ...
  test("Test additional special character combinations", async ({
    ...
    const specialCharTestCases = [
      ...
      // Note: Spaces are handled differently by the API and return 200 instead of 404
    ...
    // Test spaces separately as the API handles them differently
    const responseSpaces = await currentWeatherApiData.weatherRequest(
      request,
      BASE_URL,
      {
        q: "    ",
    ...
    // API returns 200 for spaces - documenting actual behavior
    // This might be due to the API treating spaces as empty/default input
    expect(responseSpaces.status()).toBe(200);
  });
